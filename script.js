...
initMap('map', showPlace, (level) => { $('map-breadcrumb').textContent = level.breadcrumb; $('back-to-china').hidden = level.type === 'china'; }).then((mapApi) => { mapApi.addPlaces(places); $('location-search').addEventListener('submit', (event) => { event.preventDefault(); const query = $('search-input').value.trim(); if (!query) return; mapApi.search(query, places).then((result) => { $('search-status').textContent = result.type === 'poi' ? `已定位：${result.poi.name}` : `已筛选 ${result.places.length} 个旅行地点`; }).catch((error) => { $('search-status').textContent = error.message; }); }); $('back-to-china').addEventListener('click', () => { mapApi.reset(); $('search-status').textContent = ''; $('search-input').value = ''; }); }).catch((error) => { $('map').innerHTML = '<p class="map-error">地图暂时无法加载，请检查网络后刷新页面。</p>'; console.error(error); });
